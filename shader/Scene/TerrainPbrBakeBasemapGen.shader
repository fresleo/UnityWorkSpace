Shader "Hidden/XKnight/Scene/TerrainPbrBake (Basemap Gen)"
{
    Properties
    {
        ...
        [HideInInspector] _DstBlend ("混合目标", Float) = 0.0
    }
    ...
    Subshader
    {
        ...
        Varyings Vert(Attributes input)
        {
            ...
            output.clipPos = TransformWorldToHClip(input.positionOS.xyz);
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                half sample_ao = 1, sample_metallic = 0;
                ...
                #if defined( _MASKMAP ) && !defined( _USE_PACKED_TEXTURE_MDOE )
                masks[0] = SAMPLE_TEXTURE2D(_Mask0, sampler_Mask0, input.uvSplat01.xy);
                masks[1] = SAMPLE_TEXTURE2D(_Mask1, sampler_Mask0, input.uvSplat01.zw);
                masks[2] = SAMPLE_TEXTURE2D(_Mask2, sampler_Mask0, input.uvSplat23.xy);
                masks[3] = SAMPLE_TEXTURE2D(_Mask3, sampler_Mask0, input.uvSplat23.zw);
                    #ifdef _TERRAIN_BLEND_HEIGHT
                HeightBasedSplatModify(splatControl, masks);
                    #endif
                #endif
                
                SplatmapMix(input.uvSplat01, input.uvSplat23, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS, sample_ao, sample_metallic);
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                SplatmapMix(input.uvSplat01, input.uvSplat23, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS, sampleAO, sampleMetallic);
