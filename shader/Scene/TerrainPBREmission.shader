Shader "XKnight/Scene/TerrainPBREmission"
{
    Properties
    {
        [Main(A, __, on, off)]
...
//        [SubToggle(B, _)] _EmissionOn0 ("开启自发光", Float) = 0
//        [Sub(B)] [HDR] _EmissionColor0 ("自发光颜色", Color) = (1, 1, 1, 1)
//        [Sub(B)] _EmissionColorMap0 ("自发光颜色贴图", 2D) = "White" {}
        ...
        [SubToggle(C, _)] _EmissionOn1 ("开启自发光", Float) = 0
        [Sub(C)] [HDR] _EmissionColor1 ("自发光颜色", Color) = (1, 1, 1, 1)
        [Sub(C)] _EmissionColorMap1 ("自发光颜色贴图", 2D) = "White" {}
        ...
        [SubToggle(D, _)] _EmissionOn2 ("开启自发光", Float) = 0
        [Sub(D)] [HDR] _EmissionColor2 ("自发光颜色", Color) = (1, 1, 1, 1)
        [Sub(D)] _EmissionColorMap2 ("自发光颜色贴图", 2D) = "White" {}
        ...
        [SubToggle(E, _)] _EmissionOn3 ("开启自发光", Float) = 0
        [Sub(E)] [HDR] _EmissionColor3 ("自发光颜色", Color) = (1, 1, 1, 1)
        [Sub(E)] _EmissionColorMap3 ("自发光颜色贴图", 2D) = "White" {}
    }
    ...
    SubShader
    {
        ...
        CBUFFER_START(UnityPerMaterial)
            ...
            half _EmissionOn1, _EmissionOn2, _EmissionOn3;
            half4 _EmissionColor1, _EmissionColor2, _EmissionColor3;
        CBUFFER_END
        ...
        TEXTURE2D(_EmissionColorMap1); SAMPLER(sampler_EmissionColorMap1);
        TEXTURE2D(_EmissionColorMap2);
        TEXTURE2D(_EmissionColorMap3);
        ...
        void SampleEmission(half4 splatControl, half4 uvPack0, half4 uvPack1, half2 uvPack2
            , out half3 emission)
        {
            half4 emissionColorMap1 = SAMPLE_TEXTURE2D(_EmissionColorMap1, sampler_EmissionColorMap1, uvPack1.xy);
            half4 emissionColorMap2 = SAMPLE_TEXTURE2D(_EmissionColorMap2, sampler_EmissionColorMap1, uvPack1.zw);
            half4 emissionColorMap3 = SAMPLE_TEXTURE2D(_EmissionColorMap3, sampler_EmissionColorMap1, uvPack2.xy);
            
            emission = half3(0, 0, 0)
                + splatControl.g * _EmissionOn1 * _EmissionColor1.rgb * emissionColorMap1.rgb
                + splatControl.b * _EmissionOn2 * _EmissionColor2.rgb * emissionColorMap2.rgb
                + splatControl.a * _EmissionOn3 * _EmissionColor3.rgb * emissionColorMap3.rgb;
        }
        ...
        Pass
        {
            ...
            void InitializeSurfaceData(half3 albedo, half metallic, half smoothness, half occlusion, half3 emission
            ...
            half4 SplatmapFragment(Varyings input) : SV_TARGET
            {
                ...
                half ao, metallic;
                SampleNormals(splatControl, input.pack0, input.pack1, input.pack2
                    , normalTS, ao, metallic);
                ...
                half3 emission;
                SampleEmission(splatControl, input.pack0, input.pack1, input.pack2
                    , emission);
                ...
                InitializeSurfaceData(albedo, metallic, smoothness, ao, emission
                    , surfaceData);
