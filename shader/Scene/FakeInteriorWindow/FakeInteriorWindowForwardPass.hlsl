#ifndef __FAKE_INTERIOR_WINDOW_FORWARD_PASS__
#define __FAKE_INTERIOR_WINDOW_FORWARD_PASS__
...
struct Varyings
{
    ...
    DECLARE_LIGHTMAP_OR_SH(staticLightmapUV, sh, 3);
...
Varyings vert(Attributes input)
{
    ...
    OUTPUT_SH(normalInput.normalWS.xyz, output.sh);
...
SurfaceDescription SurfaceDescriptionFunction(SurfaceDescriptionInputs IN)
{
    ...
    float crackHeightInv = 1 - crackHeight;
    float crackHeightScaledFinal = breakStepInvFinal * crackHeightInv;
    ...
    // 分割成多个窗口 + 切割掉多余的部分
    float2 tempUv = IN.uv.xy;
    float rcpWny = 1.0 / _Window_Number.y;
    float rcpWnx = 1.0 / _Window_Number.x;
    float clampFtY = clamp( ( ( _Frame_Thickness.y / _Window_Number.y ) - abs( round( tempUv.x / rcpWny ) * rcpWny - tempUv.x ) ) , 0.0 , 1.0 );
    float clampFtX = clamp( ( ( _Frame_Thickness.x / _Window_Number.x ) - abs( round( tempUv.y / rcpWnx ) * rcpWnx - tempUv.y ) ) , 0.0 , 1.0 );
    finalAlpha = step(clampFtY, 0.0) * step(clampFtX, 0.0);
    ...
    #ifdef _CRACK_MASK_ON
    float2 normalUV = IN.uv.xy * _Crack_Mask_ST.xy + _Crack_Mask_ST.zw;
    normalMap.xyz = SampleXyNormal(normalUV, TEXTURE2D_ARGS(_Crack_Mask, sampler_Crack_Mask));
    #endif // _CRACK_MASK_ON
    float4 normalLerp = lerp(normalMap, noNormal, breakStepFinal.xxxx);
    
    float metallicFinal = breakStepInv * _Metalic;
    
    float dustStep = step(0.01, dustIntensityScaled);
    float smoothnessLerp = lerp(_Smoothness, _Dust_Smoothness, dustStep);
    float smoothnessFinal = breakStepInv * smoothnessLerp;
    
    surface.BaseColor = finalLerp4.xyz;
    surface.Alpha = finalAlpha;
    surface.Emission = emissionFinal.xyz;
    
    surface.NormalTS = normalLerp.xyz;
    surface.Metallic = metallicFinal;
    surface.Smoothness = smoothnessFinal;
    surface.Occlusion = 1;
...
)
{
    ...
    // 固定为金属性工作流
    half sMetallic = saturate(surfaceDescription.Metallic);
    half3 sSpecular = 0;

    half sSmoothness = saturate(surfaceDescription.Smoothness);

    half sAlpha = saturate(surfaceDescription.Alpha);
    clip(sAlpha - 0.5); // 用它来把多余的窗户干掉
    
    half3 sNormalTS = surfaceDescription.NormalTS;
    
    SurfaceData surface;
    surface.albedo              = surfaceDescription.BaseColor;
    surface.metallic            = sMetallic;
    surface.specular            = sSpecular;
    surface.smoothness          = sSmoothness,
    surface.occlusion           = surfaceDescription.Occlusion,
    surface.emission            = surfaceDescription.Emission,
    surface.alpha               = sAlpha;
    surface.normalTS            = sNormalTS;
...
#endif // __FAKE_INTERIOR_WINDOW_FORWARD_PASS__
