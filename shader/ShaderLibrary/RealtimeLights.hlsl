#ifndef __XKNIGHT_REALTIME_LIGHTS__
#define __XKNIGHT_REALTIME_LIGHTS__
...
#include "Packages/com.garena.ta.perobjectshadow/Shaders/GPerObjectShadow.hlsl"
...
struct Light
{
    half3   direction;
    half3   color;
    float   distanceAttenuation; // 某些平台需要 full-float 精度
...
Light GetMainLight(float4 shadowCoord, float3 positionWS, half4 shadowMask)
{
    ...
    // 逐对象阴影
    light.shadowAttenuation *= PerObjectRealtimeShadow(positionWS);
...
// 填充给定 perObjectLightIndex 的光照结构体
...
// 在给定循环索引的情况下返回每个对象的索引。
// 这抽象了用于存储灯光/灯光索引的底层数据实现
...
// 填充给定循环 i 索引的 light 结构体。
// 这会将索引转换为 perObjectLightIndex
...
#endif // __XKNIGHT_REALTIME_LIGHTS__
