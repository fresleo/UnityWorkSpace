Shader "XKnight/Glass/ScreenGlassEditor"
{
    Properties
    {
        [Main(A, __, on, off)]
        _A("基础控制", Float) = 1
        [Sub(A)][Toggle]_UseEditorPreview("Editor 下使用预览纹理", Float) = 0
        [Sub(A)][NoScaleOffset][ShowIf(_UseEditorPreview, Equal, 1)]_EditorPreviewTex("Editor 预览纹理", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_EditorPreviewTex);
            SAMPLER(sampler_EditorPreviewTex);
            ...
            float4 frag(Varyings input, FRONT_FACE_TYPE cullFace : FRONT_FACE_SEMANTIC) : SV_Target
            {
                ...
                #endif
                // 勾选时采样 _EditorPreviewTex，否则采样 _ScreenCaptureTexture（本 Shader 仅用于 Editor 预览）
                float4 refracR = _UseEditorPreview >= 0.5 ? SAMPLE_TEXTURE2D_LOD(_EditorPreviewTex, sampler_EditorPreviewTex, uvR, _CaptureMipmapLevel) : SAMPLE_TEXTURE2D_LOD(_ScreenCaptureTexture, sampler_ScreenCaptureTexture, uvR, _CaptureMipmapLevel);
                float4 refracG = _UseEditorPreview >= 0.5 ? SAMPLE_TEXTURE2D_LOD(_EditorPreviewTex, sampler_EditorPreviewTex, uvG, _CaptureMipmapLevel) : SAMPLE_TEXTURE2D_LOD(_ScreenCaptureTexture, sampler_ScreenCaptureTexture, uvG, _CaptureMipmapLevel);
                float4 refracB = _UseEditorPreview >= 0.5 ? SAMPLE_TEXTURE2D_LOD(_EditorPreviewTex, sampler_EditorPreviewTex, uvB, _CaptureMipmapLevel) : SAMPLE_TEXTURE2D_LOD(_ScreenCaptureTexture, sampler_ScreenCaptureTexture, uvB, _CaptureMipmapLevel);
                refractionColor.r = refracR.r;
                refractionColor.g = refracG.g;
                refractionColor.b = refracB.b;
                ...
                // 应用雾效
                // float fogCoord = InitializeInputDataFog(float4(input.positionWS, 1.0), input.fogFactorAndVertexLight.x);
