...
#include "AirWallCompositeCore.hlsl"


float EvaluateAirWallMask(float3 positionWS)
{
    half enabled = saturate(_AirWallMaskOn);
    if (enabled < 0.5h)
        return 1.0h;

    float fullVisibleDistance = max(0.0f, _AirWallFullVisibleDistance);
    float fullHiddenDistance = max(fullVisibleDistance + 0.0001f, _AirWallFullHiddenDistance);
    float3 delta = positionWS - _AirWallRolePositionWS.xyz;
    float distanceToRole = length(delta);
    float t = saturate((distanceToRole - fullVisibleDistance) / max(0.0001f, fullHiddenDistance - fullVisibleDistance));
    return 1.0h - t;
}
...
half4 frag(v2f i, half faceId : VFACE) : SV_Target
{
    ...
    float airWallMask = EvaluateAirWallMask(i.positionWS);
    mainColor.rgb *= airWallMask;
    mainColor.a *= airWallMask;
