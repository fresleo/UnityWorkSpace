#ifndef __URP_MT_TERRAIN_VT_BUMP_PASSES__
#define __URP_MT_TERRAIN_VT_BUMP_PASSES__
...
void SplatmapMix(
    float4 uvSplat01, float4 uvSplat23,
    inout half4 splatControl,
    out half weight, out half3 mixedNormal)
{
...
half4 frag(Varyings IN) : SV_Target
{
    ...
    half occlusion = mixedMask.g;
    half metallic = mixedMask.r;
    half4 col = half4(mixedNormal.rg, occlusion, metallic);
    return weight * col;
}
...
half4 fragAdd(Varyings IN) : SV_Target
{
    ...
    clip(weight <= 0.005h ? -1.0h : 1.0h); // add专属
    mixedNormal = (mixedNormal.xyz + 1) / 2;
...
#endif //__URP_MT_TERRAIN_VT_BUMP_PASSES__
