...
namespace GRTools.Localization
{
    ...
    public class LocalizationComponentList : MonoBehaviour
    {
        ...
        private void OnLocalizationChanged(LocalizationInfo localizationInfo)
        {
            ...
            {
                if (item.localizationKey > 0 && item.component != null)
                {
                    string value = LanguageMasterManager.Singleton.GetLocalizedText(item.localizationKey, item.defaultValue);
        ...
        private void ChangeSprite(LocalizationComponentItem item, string value)
        {
            ...
            if (image != null || spriteRenderer != null)
            {
                ...
                LanguageMasterManager.Singleton.LoadLocalizationAssetAsync<Sprite>(value, sprite =>
                {
                    ...
                    {
                        LanguageMasterManager.Singleton.LoadLocalizationAssetAsync<Sprite>(item.defaultValue,
                            SpriteLoaded);
                    }
                    else
                    {
        ...
        private void ChangeTexture(LocalizationComponentItem item, string value)
        {
            ...
            if (image != null && !string.IsNullOrEmpty(value))
            {
                ...
                LanguageMasterManager.Singleton.LoadLocalizationAssetAsync<Texture>(value, texture =>
                {
                    ...
                    {
                        LanguageMasterManager.Singleton.LoadLocalizationAssetAsync<Texture>(item.defaultValue,
                            TextureLoaded);
                    }
                    else
                    {
