...
namespace GRTools.Localization
{
    public class LocalizationAssetBundleLoader : LocalizationLoader
    {
        ...
        public LocalizationAssetBundleLoader(string manifestPath = "LocalizationManifest", string bundleRootPath = "Localizations", string commonBundlePath = "Common", string commonAssetsPrefix = "Common/")
        {
            ...
            LanguageMasterManager.LocalizationChangeEvent += OnLocalizationChangeEvent;
        }
        ...
        {
            _loadingName = null;
            if (_bundleCreateRequest.assetBundle && LanguageMasterManager.Singleton.CurrentLocalizationInfo.AssetsPath == _bundleCreateRequest.assetBundle.name )
            {
        ...
        {
            ReloadAssetBundle(LanguageMasterManager.Singleton.CurrentLocalizationInfo);
            if (_assetBundle == null)
            {
                var request = _bundleCreateRequest;
                request.completed += operation =>
                {
                    if (request.assetBundle && request.assetBundle.name == LanguageMasterManager.Singleton.CurrentLocalizationInfo.AssetsPath)
                    {
                        LoadAsset();
                    }
                };
