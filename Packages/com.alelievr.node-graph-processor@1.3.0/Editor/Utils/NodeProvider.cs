...
namespace GraphProcessor
{
	public static class NodeProvider
	{
		...
		static void ProvideNodePortCreationDescription(Type nodeType, NodeDescriptions targetDescription, BaseGraph graph = null)
		{
			...
			{
				// 检查是否已经存在相同的端口描述（基于nodeType和portFieldName）
				bool alreadyExists = targetDescription.nodeCreatePortDescription.Any(desc => 
					desc.nodeType == nodeType && 
					desc.portFieldName == p.fieldName && 
					desc.isInput == input &&
					desc.portIdentifier == p.portData.identifier);
