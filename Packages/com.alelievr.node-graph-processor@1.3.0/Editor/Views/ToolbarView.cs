...
namespace GraphProcessor
{
	public class ToolbarView : VisualElement
	{
		...
		protected class ToolbarButtonData
		{
			...
			public Action<string>		textFieldCallback;
			public int				size;
		...
		protected void AddTextField(string strValue, Action<string> cb, bool left = true)
		{
			var data = new ToolbarButtonData
			{
				textValue = strValue,
			...
			foreach (var button in buttons.ToList())
			{
				...
				switch (button.type)
				{
					...
					case ElementType.TextField:
						EditorGUI.BeginChangeCheck();
						button.textValue = GUILayout.TextField(button.textValue,EditorStyles.toolbarTextField, GUILayout.Width(100));
						if(EditorGUI.EndChangeCheck() && button.textFieldCallback != null)
							button.textFieldCallback.Invoke(button.textValue);
						break;
				}
			}
		}
