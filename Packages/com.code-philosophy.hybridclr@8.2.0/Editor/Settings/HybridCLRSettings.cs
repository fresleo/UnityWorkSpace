...
namespace HybridCLR.Editor.Settings
{
    ...
    {
        [Tooltip("Encrypt global-metadata.dat")]
        public bool encryptGlobalMetadataData;

        [Tooltip("VM Random Seed")]
        public int vmSeed = 0;

        [Tooltip("Metadata Random Seed")]
        public int metadataSeed = 0;

        [Tooltip("Encryption Key")]
        public string key = "1234567890abcdef";

        public int stringEncCodeLength = 128;

        public int blobEncCodeLength = 128;

        public int userStringEncCodeLength = 128;

        public int tableEncCodeLength = 128;

        public int lazyUserStringEncCodeLength = 16;

        public int lazyTableEncCodeLength = 16;

        public int methodBodyEncCodeLength = 16;
    }
    ...
    public class HybridCLRSettings : ScriptableObject
    {
        ...
        [Tooltip("differential hybrid execution dlls")]
        public string[] differentialHybridAssemblies;

        [Tooltip("inject rule files for differential hybrid execution")]
        public string[] injectRuleFiles;
