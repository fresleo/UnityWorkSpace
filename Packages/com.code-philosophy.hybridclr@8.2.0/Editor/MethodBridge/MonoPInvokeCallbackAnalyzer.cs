        ...
        private void CollectReversePInvokeMethods()
        {
            foreach (var mod in _rootModules)
            {
                ...
                for (uint rid = 1, n = mod.Metadata.TablesStream.MethodTable.Rows; rid <= n; rid++)
                {
                    ...
                    if (!MetaUtil.IsSupportedPInvokeMethodSignature(method.MethodSig) && method.CustomAttributes.Find("HybridCLR.Runtime.UnchangedAttribute") == null)
                    {
                        Debug.LogWarning($"MonoPInvokeCallback method {method.FullName} has unsupported parameter or return type. Please check the method signature.");
                    }
