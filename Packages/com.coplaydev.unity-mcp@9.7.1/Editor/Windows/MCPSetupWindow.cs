...
namespace MCPForUnity.Editor.Windows
{
    ...
    public class MCPSetupWindow : EditorWindow
    {
        ...
        private Label autoInstallStatus;
        ...
        public void CreateGUI()
        {
            ...
            // Auto-install
            autoInstallButton = rootVisualElement.Q<Button>("auto-install-button");
            autoInstallStatus = rootVisualElement.Q<Label>("auto-install-status");
            ...
            if (autoInstallButton != null)
                autoInstallButton.clicked += OnAutoInstallClicked;
        ...
        private void OnAutoInstallClicked()
        {
            if (autoInstallButton == null)
                return;

            if (!DependencyInstaller.IsInstallNeeded())
            {
                EditorUtility.DisplayDialog(
                    "Already Installed",
                    "All dependencies are already satisfied. Click Refresh to verify.",
                    "OK");
                return;
            }

            autoInstallButton.SetEnabled(false);
            autoInstallStatus.text = "Installing... A terminal window will open. Close it when done, then click Refresh.";
            autoInstallStatus.style.display = DisplayStyle.Flex;

            try
            {
                DependencyInstaller.RunInstall();
            }
            catch (System.Exception ex)
            {
                EditorUtility.DisplayDialog(
                    "Install Error",
                    $"Failed to start automatic installer:\n{ex.Message}",
                    "OK");
                autoInstallButton.SetEnabled(true);
                autoInstallStatus.style.display = DisplayStyle.None;
            }
        }
    }
}
