#ifndef EXPONENTIAL_HEIGHT_FOG
#define EXPONENTIAL_HEIGHT_FOG

// todo: 因为有 cg 语言的 shader 引用这个，所以直接导包会报错
//#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"

#if defined(_HEIGHT_FOG)
	#if defined(_RECORDING_QUALITY) || ( !defined(SHADER_API_MOBILE) && !defined(SHADER_API_SWITCH) ) // 录制画质或非移动平台
#define UBPA_FOG_COORDS(idx)
#define UBPA_TRANSFER_FOG(v2f, positionWS)
#define UBPA_APPLY_FOG(v2f, pixelColor) \
	float4 _fogCoord = GetExponentialHeightFog(-GetWorldSpaceViewDir(v2f.positionWS)); \
	ApplyHeightFog(pixelColor, _fogCoord);
#define UBPA_APPLY_FOG_COLOR(v2f, pixelColor, fogColor) \
	float4 _fogCoord = GetExponentialHeightFog(-GetWorldSpaceViewDir(v2f.positionWS)); \
	ApplyHeightFogColor(pixelColor, _fogCoord, fogColor);
	#else

#define UBPA_FOG_COORDS(idx) \
	float4 _fogCoord : TEXCOORD##idx;
#define UBPA_TRANSFER_FOG(v2f, positionWS) \
	v2f##._fogCoord = GetExponentialHeightFog(-GetWorldSpaceViewDir(positionWS));
#define UBPA_APPLY_FOG(v2f, pixelColor) \
	ApplyHeightFog(pixelColor, v2f._fogCoord);
#define UBPA_APPLY_FOG_COLOR(v2f, pixelColor, fogColor) \
	ApplyHeightFogColor(pixelColor, v2f._fogCoord, fogColor);
	#endif
#else
...
float CalculateLineIntegralShared(float FogHeightFalloff, float RayDirectionY, float RayOriginTerms)
{
	...
	float LineIntegral = (1.0f - exp2(-Falloff)) / (Falloff + 1e-4);
...
half4 GetExponentialHeightFog(float3 WorldPositionRelativeToCamera) // camera to vertex
{
	...
	if (InscatteringLightDirection.w >= 0)
	{
		...
		float powLeft = saturate(dot(CameraToReceiverNormalized, InscatteringLightDirection.xyz)) + 1e-4; // 保护一下，避免出现 pow(0, e) 的情况
		half3 DirectionalLightInscattering = DirectionalInscatteringColor.xyz * pow(powLeft, DirectionalInscatteringColor.w);
...
void ApplyHeightFog(inout half3 pixelColor, float4 fogCoord)
{
	pixelColor.rgb = pixelColor.rgb * fogCoord.a + fogCoord.rgb;
}

void ApplyHeightFog(inout half4 pixelColor, float4 fogCoord)
{
	ApplyHeightFog(pixelColor.rgb, fogCoord);
}

void ApplyHeightFogColor(inout half3 pixelColor, float4 fogCoord, half3 fogColor)
{
	fogCoord.rgb = fogColor;
	ApplyHeightFog(pixelColor, fogCoord);
}

void ApplyHeightFogColor(inout half4 pixelColor, float4 fogCoord, half3 fogColor)
{
	ApplyHeightFogColor(pixelColor.rgb, fogCoord, fogColor);
}

#endif// !EXPONENTIAL_HEIGHT_FOG
