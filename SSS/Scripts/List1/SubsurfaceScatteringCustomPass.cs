...
public class SubsurfaceScatteringCustomPass : CustomPass
{
    ...
    public Texture2D PreIntegratedTex; //预积分图
    ...
    [Tooltip("整体散射半径倍率")] public float radiusMultiplier = 1.0f;

    [Tooltip("kernel 半径是 mm，这里是 1 世界单位 = ? 米（米/世界单位）。默认 1。")]
    public float worldScale = 1.0f;

    [Tooltip("单轴最大像素偏移，限制过大模糊")] public float maxRadiusPixels = 64.0f;
    [Tooltip("深度剔除强度，越大跨深度越容易被剔除")] public float depthFalloff = 1.0f;
    ...
    static class SID
    {
        ...
        public static readonly int rtHandleScale = Shader.PropertyToID("_SSSRtHandleScale");
    }
    ...
    protected override void Execute(CustomPassContext ctx)
    {
        ...
        var camType = ctx.hdCamera.camera.cameraType;
        ...
        Debug.Log("kernelN" + kernelN + ", zParams:" + zParams + ", pixelScale:" + pixelScale);
    ...
    void PushGlobals(CommandBuffer cmd)
    {
        ...
        if (l != null && l.type == LightType.Directional)
        {
            ...
            cmd.SetGlobalVector(SID.rtHandleScale, rtHandleScale);
        }
        else
        {
